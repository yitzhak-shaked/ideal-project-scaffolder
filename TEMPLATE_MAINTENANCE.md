...
## The feature pipeline (`include_feature_pipeline`)

When `include_feature_pipeline` is true (default), the template emits an
end-to-end feature/bug workflow. It is **canonical here and synced down to the
standalone `ideal-workflow` plugin** by `scripts/sync-plugin.py` — edit the
template, then re-sync; don't hand-edit the plugin's agents/hooks.

Pieces (all gated by `include_feature_pipeline`; the Claude-only pieces also
gate on `ai_agent in ['claude','universal']`):

- **Split role agents** under `{{ ai_folder }}/agents/`: `test-writer`,
  `implementer`, `verifier`, `debugger`, `documenter`. They reuse the existing
  `ddd-architect` (as planner) and `code-reviewer` (as reviewer). When the
  pipeline is on, `tdd-driver` is gated **off** (its filename is
  `{% raw %}{% if not include_feature_pipeline %}tdd-driver.md{% endif %}.jinja{% endraw %}`)
  because the split set replaces it.
- **Playbook**: `{{ ai_folder }}/instructions/general/feature-pipeline.md` — the
  phase map + the per-project runtime config in its YAML front-matter
  (`spec_kit` seeded from `use_spec_kit`, `loop_bound`, `plan_grill`,
  `security_gate`). Agents and the orchestrator read this first.
- **Grilling skill**: project-local `{{ ai_folder }}/skills/grilling/` (no
  superpowers equivalent; hand-authored, not vendored).
- **Orchestrator** (Claude/Universal only): `.claude/commands/feature.md` — the
  `/feature` slash command.
- **Hard gate** (Claude/Universal only): `.claude/hooks/block-locked-tests.py`
  (PreToolUse, blocks test edits while `.pipeline/tests.lock` exists) +
  `autoformat.py` (PostToolUse). Registered via the `hooks` block in
  `.claude/settings.json.jinja`, which also carries the pipeline `permissions`
  allowlist (so routine commands don't prompt). Both are plain `.py` (content is
  template-free) with Jinja-gated filenames. When `sync-plugin.py` copies them
  into the standalone plugin it stamps a `GENERATED by …` header; the template
  sources stay header-free.

Skills are referenced by their vendored/project-local **directory name**
(`test-driven-development`, `systematic-debugging`, `grilling`, …), never a
global plugin namespace — the rendered project disables the global superpowers
plugin via `enabledPlugins`.

`_hooks/post_generation.py` prunes empty `commands/` and `hooks/` directories
(via `PRUNE_IF_EMPTY_DIRS`), so a pipeline-off render — or any non-Claude agent
target — leaves no hollow `.claude/commands/` or `.claude/hooks/` behind.
